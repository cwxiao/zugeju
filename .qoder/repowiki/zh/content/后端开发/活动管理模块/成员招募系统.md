...
**所做更改**
- 新增邀请接受/拒绝接口的详细实现说明
- 完善成员状态管理的完整状态枚举和处理逻辑
- 增强成员关系维护的数据一致性保障机制
- 补充通知机制的完整实现细节
- 更新API设计以反映新增的decline接口
...
## 引言
本文件面向成员招募系统的技术实现，围绕邀请机制、邀请链接、状态跟踪、角色管理、成员状态、关系维护、API 设计以及用户体验与一致性保障进行系统化梳理。系统现已增强邀请接受/拒绝功能，提供完整的成员招募闭环。目标是帮助开发者快速理解并扩展该模块，同时为产品与测试同学提供清晰的参考。
...
C-->>U : "返回结果"
U->>C : "POST /api/activities/{id}/decline"
C->>S : "decline(userId, activityId)"
S->>S : "校验活动存在"
S->>M : "decline(activityId, userId)"
M-->>S : "返回影响行数"
S-->>C : "CreateActivityResponse"
C-->>U : "返回结果"
```
...
### 成员状态管理
- 状态枚举
  - join_status 默认 joined，支持 quit（拒绝/退出）、waiting（候补）等状态。
  - invite_status 支持 invited、viewed、accepted、declined、expired 等状态。
  - invite_event 支持 viewed、accepted、declined、reopened、joined、quit 等事件类型。
- 处理逻辑
...
## 结论
成员招募系统以活动为中心，通过邀请链路、成员关系与通知事件三部分协同，实现了从邀请到加入/拒绝再到通知的完整闭环。系统采用幂等与事务保障一致性，结合数据库索引与轮询机制提升可用性。新增的邀请接受/拒绝接口进一步完善了用户体验，未来可在角色转换、成员移除、邀请批量发送等方面进一步扩展。
