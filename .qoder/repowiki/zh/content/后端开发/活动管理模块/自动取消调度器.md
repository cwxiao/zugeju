...
## 更新摘要
**所做更改**
- 新增通知事件集成章节，详细说明通知事件表结构与去重机制
- 更新调度器工作流程图，反映新的通知事件处理逻辑
- 完善数据库表结构说明，包含通知事件表的完整字段定义
- 增强监控与告警章节，涵盖通知事件发送状态跟踪
- 更新故障排查指南，增加通知事件相关问题诊断方法
...
**更新** 本次更新重点完善了通知事件集成机制，新增了通知事件表结构说明和去重逻辑分析。
...
### 通知事件表结构：ActivityNotificationEventMapper
- 表结构设计：包含活动ID、目标用户ID、触发用户ID、事件类型、标题、内容、页面路径、发送状态等字段。
- 索引策略：为用户ID、发送状态、创建时间建立复合索引，优化查询性能。
- 状态管理：支持 pending、sent、skipped、failed 四种发送状态。
- 去重机制：通过事件类型和目标用户的唯一性约束，防止重复通知。
...
- 通知事件重复发送
  - 检查去重逻辑是否正常工作，确认事件类型和目标用户的唯一性约束。
  - 验证通知事件表中的重复记录清理机制。
...
- 通知事件监控
  - 监控通知事件表的增长趋势，及时发现异常的大量通知积压。
  - 跟踪发送状态的变化，确保通知事件能够正常流转。
...
- 验证去重机制
  - 连续触发同一活动的提醒逻辑，确认只生成一条提醒事件。
  - 验证取消事件的去重逻辑，确保不会重复记录已完成的取消事件。
